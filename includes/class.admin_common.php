    ...
    class WPC_Client_Admin_Common extends WPC_Client_Common {
        ...
        */
        function get_plugin_admin_notices() {
            ...
            if ( current_user_can( 'administrator' ) ) {
                ...
                $wpc_admin_notices = $this->cc_get_settings( 'lite_admin_notices' );
                $lite_install = get_option( 'wp_client_lite_install' );

                if ( $lite_install && ( time() - ( 3600 * 24*7 ) ) > $lite_install &&  ( !isset( $wpc_admin_notices['time'] ) || ( time() - ( 3600 * 48 ) ) > $wpc_admin_notices['time'] ) ) {
                    $wpc_admin_notices['time'] = time();
                    $wpc_admin_notices['contents'] = $this->remote_download( "https://webportalhq.com/_remote/wpc-lite/wpc_lite_admin_notice.txt" );

                    do_action( 'wp_client_settings_update', $wpc_admin_notices, 'lite_admin_notices' );
                }
                ...
                if ( isset( $wpc_admin_notices['contents'] ) && '' != $wpc_admin_notices['contents'] ) {
                    ...
                    $wpc_admin_notices['contents'] = str_replace( '{plugin_title}', $this->plugin['title'], $wpc_admin_notices['contents'] );

                    $wpc_admin_notices['contents'] = json_decode( $wpc_admin_notices['contents'], true );

                    if ( is_array( $wpc_admin_notices['contents'] ) ) {

                        $wpc_dismiss_admin_notice = $this->cc_get_settings( 'dismiss_lite_admin_notice' );
                        $wpc_dismiss_admin_notice = is_array( $wpc_dismiss_admin_notice ) ? $wpc_dismiss_admin_notice : array();

                        foreach( $wpc_admin_notices['contents'] as $content ) {
                            ...
                            if ( isset( $content['message'] ) && version_compare( WPC_CLIENT_LITE_VER, $content['from_plugin_version'], '>=' ) && version_compare( WPC_CLIENT_LITE_VER, $content['last_plugin_version'], '<=' ) ) {

                                ?>
                                <div id="message" class="updated wpc_notice" style="clear: both; float: left">
                                    <p>
        ...
        */
        function get_plugin_logo_block() {
            echo $html = '<div class="wpc_logo">' . $this->plugin['logo_content'] . '</div><div id="wpc_get_pro"><a href="https://webportalhq.com/link/get-pro-version/" title="Get WP-Client Pro Version" target="_blank">Get <span>PRO</span></a></div><hr />';
            $this->get_plugin_admin_notices();
            return '';
        }
